...
\label{sec:Meeting 5}
  ...
  \begin{itemize}
    ...
    \item It is also hard to find variables that only affect training, but not wages
    \item Thus, the effect of private-sector training is typically overstated
    \item In this paper, the authors compare the wages of workers that took training to wages of workers who \textbf{wanted} to take training, but could not do so due to some random reason
...
% section Meeting 5 (end)
  ...
  \label{sec:Meeting 7}
    \subsection{(Tomlinson, 1997) "Measuring competence and knowledge using employee surveys: evidence using the British Skills Survey of 1997"} % (fold)
      \subsubsection{Approach}
